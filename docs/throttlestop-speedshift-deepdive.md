...
* **`160` (`0xA0`)**: **62.5% 절전 (AC 전원 추천★)** - C0% 부하량에 반응하여 클럭을 비례적으로 아껴 씀
* **`192` (`0xC0`)**: **75% 절전 (DC 배터리 추천★)** - 16진수 `0xC0` 75% 절전 기준점. CPU 소비전력 **0.9W / 38°C** 극저전력 안착!
* **`200` (`0xC8`)**: 약 78% 절전 - 부스트 발작 90% 이상 억제
* **`224` (`0xE0`)**: 87.5% 절전 - 90%+ 고부하 시에만 반응
* **`255` (`0xFF`)**: 100% 절전 - 부스트 완전 억제
...
윈도우 스케줄러는 미세한 작업도 빠르게 처리하고 C-State(휴식)로 가기 위해 클럭을 튀어 오르게 합니다. `EPP = 160~192` 세팅은 이러한 스케줄러의 미세 자극을 완충하여 쾌적한 발열 케어를 가능하게 합니다.
